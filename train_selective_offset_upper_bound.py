...
def train_once(args, batch_size):
    ...
    def verify_and_lock_projection_on_train_start(trainer):
        """Verify Trainer reconstruction, then enforce route and freezing."""
        modules = [
            module
        ...
        if len(modules) != 1:
            raise RuntimeError(
                "Trainer 中应有一个 IRGuidedSelectiveOffset，实际为 {}".format(
                    len(modules)
                )
            )
        ...
        for name, expected in expected_projection.items():
            if name not in current_state:
                raise RuntimeError(f"Trainer 中缺少 projection 参数: {name}")
            difference = (
        ...
        if max_difference >= 1e-7:
            raise RuntimeError(
                "Trainer 重建后 projection 权重发生变化，"
                f"max_diff={max_difference:.8g}"
            )

        # Some Ultralytics versions re-enable floating-point parameters while
        # setting up the Trainer. Re-apply both non-state settings here.
        ...
        if projection_trainable:
            raise RuntimeError("projection 在训练开始时仍为可训练状态")
...
def parse_args():
    ...
    parser.add_argument("--project", type=str, default="runs/dronevehicle/obb/offset")
    parser.add_argument(
        "--name",
        type=str,
        default="offset_center_p3_e10_freeze23_s42",
    )
