...
def parse_args():
    ...
    parser.add_argument("--name", type=str, default="v2", help="本次实验保存的子目录名")
    parser.add_argument("--exist-ok", action="store_true", default=True, help="允许覆盖已存在的保存目录")
    parser.add_argument("--save-json", action="store_true", help="保存 COCO json 结果（若支持）")
    parser.add_argument("--save-txt", action="store_true", default=True, help="保存预测 txt")
    parser.add_argument("--save-conf", action="store_true", default=True, help="保存 txt 时附带 conf")
    parser.add_argument("--save-hybrid", action="store_true", help="保存 hybrid labels（若底层支持）")
    parser.add_argument("--plots", action="store_true", help="保存 PR/F1/混淆矩阵等图")
...
def main():
    ...
    print("\n即将调用 model.val(...)，关键保存参数如下：")
    print("project    :", project)
    print("name       :", args.name)
    print("exist_ok   :", args.exist_ok)
    print("save_json  :", args.save_json)
    print("save_txt   :", args.save_txt)
    print("save_conf  :", args.save_conf)
    print("save_hybrid:", args.save_hybrid)
    print("plots      :", args.plots)
    print("说明：若你的双流 validator 支持 save_txt，则预测 txt 一般会出现在保存目录下的 labels/ 中。\n")
    ...
    print(f"- {project / args.name / 'labels'}")
