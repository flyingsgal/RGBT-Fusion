...
3) Evaluate the saved projection on the complete validation split without
   retraining it:

    python train_p3_common_space_probe.py \
      --eval-only \
      --weights /path/to/add/best.pt \
      --projection-weights runs/p3_common_space_probe/first_run/projection_probe.pt \
      --data /path/to/dronevehicle_obb.yaml \
      --val-state-csv /path/to/val_matched_state_index.csv \
      --output runs/p3_common_space_probe/full_val \
      --device 0 --batch 8 --imgsz 640 --max-val-images 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--eval-only",
        action="store_true",
        help="Load --projection-weights and evaluate val without training.",
    )
    ...
    parser.add_argument("--output", type=str, default="runs/p3_common_space_probe/full_val_eval")
...
def validate_args(args: argparse.Namespace) -> None:
    if args.self_test:
        return
    required = ["weights", "data", "val_state_csv"]
    if args.eval_only:
        required.append("projection_weights")
    else:
        required.append("train_state_csv")
    ...
    if args.eval_tau <= 0:
        raise ValueError("--eval-tau must be positive.")
    if not args.eval_only and (
        args.epochs <= 0 or args.lr <= 0 or args.train_tau <= 0
    ):
        raise ValueError("epochs/lr/train-tau values must be positive.")
...
def main() -> None:
    ...
    if args.eval_only:
        ...
        required_checkpoint_keys = {
            ...
            "input_dim",
            "embed_dim",
            "train_global_mean_offset_cell",
        }
        missing_checkpoint_keys = sorted(
            required_checkpoint_keys.difference(checkpoint.keys())
        )
        if missing_checkpoint_keys:
            raise ValueError(
                "Projection checkpoint is missing keys: {}".format(
        ...
        if mismatches:
            raise ValueError(
                "Evaluation geometry must match the projection checkpoint: {}".format(
                    "; ".join(mismatches)
                )
            )
        args.embed_dim = int(checkpoint["embed_dim"])
    ...
    if args.eval_only:
        print("Evaluation only: the saved projection will not be updated.")
    else:
    ...
    init_split = "val" if args.eval_only else "train"
    run_validator(yolo, InitCallback(), args, init_split)
    channels = int(matcher_holder["channels"])
    feature_device = rgb_capture.tensor.device
    if checkpoint is not None and channels != int(checkpoint["input_dim"]):
        raise ValueError(
            "P3 channel mismatch: detector={} projection_checkpoint={}".format(
                channels, checkpoint["input_dim"]
            )
        )
    ...
    try:
        if args.eval_only:
            ...
            train_global_mean = np.asarray(
                checkpoint["train_global_mean_offset_cell"], dtype=float
            )
            print("Loaded projection: {}".format(Path(args.projection_weights).resolve()))
        else:
        ...
        assert train_global_mean is not None
        decision = decision_summary(diagnostics, train_global_mean, args.stride)
        checkpoint_args = checkpoint.get("args", {}) if checkpoint is not None else {}
        train_state_source = args.train_state_csv or checkpoint_args.get(
            "train_state_csv"
        )
        decision.update(
            {
                ...
                "projection_weights": (
                    str(Path(args.projection_weights).resolve())
                    if args.projection_weights
                    else None
                ),
                "eval_only": bool(args.eval_only),
                "train_state_csv": (
                    str(Path(train_state_source).resolve())
                    if train_state_source
                    else None
                ),
